...
void Server_process_request(Server *serv, User *usr)
{
	...
	while (cc_array_iter_next(&itr, (void **)&message) != CC_ITER_END)
	{
		...
		if (message->command)
		{
			...
			{
				User_add_msg(usr, make_reply(":%s " ERR_UNKNOWNCOMMAND_MSG, serv->hostname, usr->nick, message->command));
			}
		}
...
void Server_reply_to_NICK(Server *serv, User *usr, Message *msg)
{
	...
	{
		User_add_msg(usr, make_reply(":%s " ERR_NICKNAMEINUSE_MSG, serv->hostname, msg->params[0]));
		return;
	}
...
void Server_reply_to_USER(Server *serv, User *usr, Message *msg)
{
	...
	{
		User_add_msg(usr, make_reply(":%s " ERR_ALREADYREGISTRED_MSG, serv->hostname, usr->nick));
		return;
	}
...
void Server_reply_to_PRIVMSG(Server *serv, User *usr, Message *msg)
{
	...
	if (msg->n_params == 0)
	{
		User_add_msg(usr, make_reply(":%s " ERR_NORECIPIENT_MSG, serv->hostname, usr->nick));
		return;
	}
	...
	if (msg->n_params > 1)
	{
		User_add_msg(usr, make_reply(":%s " ERR_TOOMANYTARGETS_MSG, serv->hostname, usr->nick));
		return;
	}
	...
	if (!target_username)
	{
		User_add_msg(usr, make_reply(":%s " ERR_NOSUCHNICK_MSG, serv->hostname, usr->nick, target_nick));
		return;
	}
	...
	if (!target_sock)
	{
		User_add_msg(usr, make_reply(":%s " RPL_AWAY_MSG, serv->hostname, usr->nick, target_nick));
		return;
	}
