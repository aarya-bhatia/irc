    ...
    while (List_size(conn->incoming_messages) > 0)
    {
        ...
        if (!found)
        {
            ...
            else if (!strcmp(message.command, "PRIVMSG"))
            {
                ...
                if (*message.params[0] == '#')
                {
                    Server_message_channel(serv, peer->name, message.params[0] + 1, message_str);
                }
                else
                {
                    Server_message_user(serv, peer->name, message.params[0], message_str);
                }
            }
...
bool Server_add_peer(Server *serv, const char *name, const char *port)
{
    ...
    int fd =
        connect_to_host(peer_info.peer_host, port ? port : peer_info.peer_port);
