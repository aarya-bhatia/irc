...
static UserRequestHandler user_request_handlers[] = {
	...
	{"JOIN", Server_handle_JOIN}, {"PART", Server_handle_PART},
	{"NAMES", Server_handle_NAMES}, {"TOPIC", Server_handle_TOPIC},
	{"LUSERS", Server_handle_LUSERS}, {"HELP", Server_handle_HELP},
...
Server *Server_create(const char *name)
{
	...
	serv->connections = ht_alloc_type(INT_TYPE, SHALLOW_TYPE);	/* Map<int,
									 * Connection
									 * *> */
	serv->name_to_peer_map = ht_alloc_type(STRING_TYPE, SHALLOW_TYPE);	/* Map<string,
										 * Peer
										 * *>
										 */
	serv->nick_to_serv_name_map = ht_alloc(STRING_TYPE, STRING_TYPE);	/* Map<string,
										 * string>
										 */
	serv->nick_to_user_map = ht_alloc_type(STRING_TYPE, SHALLOW_TYPE);	/* Map<string,
										 * User*>
										 */

	serv->name_to_channel_map = load_channels(CHANNELS_FILENAME);	/* Map<string,
									 * Channel
									 * *>
									 */
...
void Server_process_request_from_user(Server * serv, Connection * conn)
{
	...
	// Iterate over the request messages and add response message(s) to
	// user's
	...
	for (size_t i = 0; i < Vector_size(messages); i++) {
		...
		for (size_t j = 0; j < sizeof(user_request_handlers)
		     / sizeof(user_request_handlers[0]); j++) {
		...
		if (!usr->registered) {
			char *reply =
			    Server_create_message(serv,
						  "451 %s :Connection not registered",
						  usr->nick);
	...
	if (channel) {
		...
		while (ht_iter_next(&itr, NULL, (void **)&member)) {
			if (member->registered && !member->quit) {
				List_push_back(member->msg_queue,
					       strdup(message));
			}
		}
	}
...
void Server_process_request_from_peer(Server * serv, Connection * conn)
{
	...
	while (List_size(conn->incoming_messages) > 0) {
		...
		for (size_t i = 0; i < sizeof(peer_request_handlers)
		     / sizeof(peer_request_handlers[0]); i++) {
		...
		if (!found) {
			...
			if (!strcmp(message.command, "NICK")) {
				ht_set(serv->nick_to_serv_name_map,
				       message.params[0], peer->name);
				Server_relay_message(serv, peer->name,
						     message_str);
			}
...
bool Server_add_connection(Server * serv, Connection * connection)
{
	...
	if (fcntl(connection->fd, F_SETFL,
		  fcntl(connection->fd, F_GETFL) | O_NONBLOCK)
	    != 0) {
		perror("fcntl");
		return false;
	}
	...
	struct epoll_event ev
	    = {.data.fd = connection->fd,.events = EPOLLIN | EPOLLOUT };
...
void Server_remove_connection(Server * serv, Connection * connection)
{
	...
	if (connection->conn_type == USER_CONNECTION) {
		...
		for (size_t i = 0; i < Vector_size(usr->channels); i++) {
			...
			Channel *channel =
			    ht_get(serv->name_to_channel_map, name);
			if (channel) {
				Channel_remove_member(channel, usr);
			}
		}

		Server_relay_message(serv, serv->name,
				     Server_create_message(serv, "QUIT :%s",
							   "closing link"));
		...
		if (peer->name) {
			ht_remove(serv->name_to_peer_map, peer->name, NULL,
				  NULL);
		}
		...
		Server_relay_message(serv, serv->name,
				     Server_create_message(serv, "SQUIT %s :%s",
							   peer->name,
							   "closing link"));
