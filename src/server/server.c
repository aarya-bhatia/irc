...
Server *Server_create(const char *name)
{
	...
	serv->name_to_channel_map
	  = load_channels(CHANNELS_FILENAME); /* Map<string,
										   * Channel
										   * *>
										   */
	...
	for(size_t i = 0; i < Vector_size(messages); i++)
		{
			...
			for(size_t j = 0; j < sizeof(user_request_handlers)
									/ sizeof(user_request_handlers[0]);
				j++)
				{
			...
			if(!usr->registered)
				...
				{
					char *reply = Server_create_message(
					  serv, "421 %s %s :Unknown command", usr->nick,
					  message->command);
					List_push_back(conn->outgoing_messages, reply);
				}
	...
	while(List_size(conn->incoming_messages) > 0)
		{
			...
			for(size_t i = 0; i < sizeof(peer_request_handlers)
									/ sizeof(peer_request_handlers[0]);
				i++)
				{
							...
							if(*message.params[0] == '#')
								{
									Server_message_channel(
									  serv, peer->name, message.params[0] + 1,
									  message_str);
								}
							else
								{
