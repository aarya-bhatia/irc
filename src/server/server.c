...
Server *Server_create(const char *name) {
  ...
  log_info("Server \"%s\" is running on port %s at %s", serv->name, serv->port,
           serv->hostname);
  return serv;
}
...
void Server_accept_all(Server *serv) {
  ...
  while (1) {
    ...
    Connection *conn =
        Connection_alloc(conn_sock, (struct sockaddr *)&client_addr, addrlen);
...
void Server_process_request_from_user(Server *serv, Connection *conn) {
  ...
  for (size_t i = 0; i < Vector_size(messages); i++) {
    ...
    if (!message->command) {
      ...
      char *reply = Server_create_message(serv, "421 %s %s :Unknown command",
  ...
  for (size_t i = 0; i < Vector_size(messages); i++) {
    ...
    } else if (!strcmp(message->command, "QUIT")) // A user behind peer has quit
    {
      ...
      List_push_back(peer->msg_queue,
                     Server_create_message(serv, "ERROR :Closing Link: %s",
                                           conn->hostname));
      peer->quit = true;
      break;
    } else if (!strcmp(message->command, "TEST_LIST_SERVER")) {
      ...
      if (message->n_params > 0 && message->body) {
        ...
        if (list_data) {
          log_debug("Sent TEST_LIST_SERVER reply for %s to fd %d", target,
                    list_data->conn->fd);
      ...
      if (message->n_params > 0 && message->body) {
        ...
        if (list_data) {
          ...
          if (ht_size(list_data->pending) == 0) {
            log_debug("Sent TEST_LIST_SERVER_END reply for %s to %d", target,
                      list_data->conn->fd);
            List_push_back(
                list_data->conn->outgoing_messages,
                Server_create_message(serv, "902 %s :End of TEST_LIST_SERVER",
                                      target));
      ...
      if (ht_contains(serv->name_to_peer_map,
                      server_name)) { // A new server has joined the network
                                      // behind the current peer
      ...
      log_warn("removed nick %s", nick);
                   ...
                   message->command,
                   "NICK")) // A new user was registered behind the peer server
    {
      char *nick = message->params[0];
      assert(nick);
      ...
      if (ht_contains(serv->nick_to_serv_name_map, nick)) {
        List_push_back(
            peer->msg_queue,
            Server_create_message(serv, "KILL %s :nickname collision", nick));
        ht_remove(serv->nick_to_serv_name_map, nick, NULL, NULL);
...
bool _remove_nick_for_peer(char *nick, char *name,
  ...
  if (!strcmp(name, filter_arg->peer->name)) {
    ...
    Server_broadcast_message(filter_arg->serv,
                             make_string(":%s!*@* QUIT :closing link", nick));
    return true;
  }
...
bool _remove_server_for_peer(char *name, Peer *peer,
  ...
  if (!strcmp(peer->name, filter_arg->peer->name)) {
    Server_broadcast_message(filter_arg->serv,
                             Server_create_message(filter_arg->serv,
                                                   "SQUIT %s :closing link",
                                                   name));
    return true;
  }
...
void Server_remove_connection(Server *serv, Connection *connection) {
  ...
  if (connection->conn_type == USER_CONNECTION) {
    ...
    Server_broadcast_message(serv, User_create_message(usr, "QUIT :%s",
                                                       usr->quit_message
                                                           ? usr->quit_message
                                                           : "Closing Link"));
    User_free(usr);
