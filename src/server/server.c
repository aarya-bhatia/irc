...
typedef struct _UserRequestHandler
{
  ...
  void (*handler)(Server *, User *, Message *); /* request
                                                 * handler
                                                 * function
                                                 */
} UserRequestHandler;
...
{
  const char *name;                             /* command name */
  void (*handler)(Server *, Peer *, Message *); /* request
                                                 * handler
                                                 * function
                                                 */
} PeerRequestHandler;
...
Server *Server_create(const char *name)
{
  ...
  serv->name_to_peer_map
    = ht_alloc_type(STRING_TYPE, SHALLOW_TYPE); /* Map<string,
                                                 * Peer
                                                 * *>
                                                 */
  serv->nick_to_serv_name_map
    = ht_alloc(STRING_TYPE, STRING_TYPE); /* Map<string,
                                           * string>
                                           */
  ...
  for(size_t i = 0; i < Vector_size(messages); i++)
    {
      ...
      for(size_t j = 0;
          j < sizeof(user_request_handlers) / sizeof(user_request_handlers[0]);
          j++)
        {
      ...
      if(!usr->registered)
        ...
        {
          char *reply = Server_create_message(
            serv, "421 %s %s :Unknown command", usr->nick, message->command);
          List_push_back(conn->outgoing_messages, reply);
        }
  ...
  while(List_size(conn->incoming_messages) > 0)
    {
      ...
      for(size_t i = 0;
          i < sizeof(peer_request_handlers) / sizeof(peer_request_handlers[0]);
          i++)
        {
      ...
      if(!found)
        {
          ...
          if(!strcmp(message.command, "NICK"))
            {
              ht_set(serv->nick_to_serv_name_map, message.params[0],
                     peer->name);
              Server_relay_message(serv, peer->name, message_str);
            }
              ...
              if(*message.params[0] == '#')
                {
                  Server_message_channel(serv, peer->name,
                                         message.params[0] + 1, message_str);
                }
              else
                {
  ...
  if(connection->conn_type == USER_CONNECTION)
    {
      ...
      Server_relay_message(
        serv, serv->name,
        Server_create_message(serv, "QUIT :%s", "closing link"));
