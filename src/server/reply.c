                    ...
                    User *target_usr)
{
  List_push_back(usr->msg_queue,
                 Server_create_message(
                   serv, RPL_WHOREPLY_MSG, usr->nick, target_channel->name,
...
void Server_handle_MOTD(Server *serv, User *usr, Message *msg)
{
  ...
  List_push_back(usr->msg_queue, Server_create_message(
                                   serv, "375 %s :- %s Message of the day - ",
                                   usr->nick, serv->name));
...
void Server_handle_PRIVMSG(Server *serv, User *usr, Message *msg)
{
  ...
  if(msg->n_params == 0)
    {
      List_push_back(usr->msg_queue, Server_create_message(
                                       serv, ERR_NORECIPIENT_MSG, usr->nick));
      return;
    }
...
void Server_handle_PART(Server *serv, User *usr, Message *msg)
{
  ...
  char *reason = msg->body ? strdup(msg->body)
                           : make_string("%s is leaving channel %s", usr->nick,
                                         channel->name);
...
void Server_handle_INFO(Server *serv, User *usr, Message *msg)
{
  ...
  List_push_back(usr->msg_queue, Server_create_message(serv, "371 %s :%s",
                                                       usr->nick, serv->info));
  List_push_back(
    usr->msg_queue,
