	...
	if(motd)
		{
			List_push_back(
			  usr->msg_queue,
			  Server_create_message(serv, RPL_MOTD_MSG, usr->nick, motd));
		}
	else
		{
			List_push_back(usr->msg_queue, Server_create_message(
											 serv, ERR_NOMOTD_MSG, usr->nick));
		}
	...
	if(!usr->registered && usr->nick_changed && usr->username && usr->realname)
		{
			...
			// <realname>
			char *message = Server_create_message(
			  serv, "%s 1 %s %s 1 + :%s", usr->nick, usr->username,
...
void Server_handle_NICK(Server *serv, User *usr, Message *msg)
{
	...
	if(ht_contains(serv->nick_to_user_map, new_nick))
		{
			List_push_back(usr->msg_queue,
						   Server_create_message(serv, ERR_NICKNAMEINUSE_MSG,
												 msg->params[0]));
			return;
		}
	...
	if(usr->registered)
		{
			List_push_back(usr->msg_queue,
						   Server_create_message(
							 serv, ERR_ALREADYREGISTRED_MSG, usr->nick));
			return;
		}
...
void Server_handle_WHO(Server *serv, User *usr, Message *msg)
{
	...
	if(mask[0] == '#')
	...
	else
		{
			...
			if(other_user)
				{
					for(size_t i = 0; i < Vector_size(other_user->channels);
						i++)
						{
							Channel *channel
	...
	List_push_back(usr->msg_queue, Server_create_message(
									 serv, RPL_ENDOFWHO_MSG, usr->nick, ""));
}
			...
			while(ht_iter_next(&itr, NULL, (void **)&channel))
				{
					...
					List_push_back(usr->msg_queue,
								   Server_create_message(
									 serv, RPL_LIST_MSG, usr->nick,
									 channel->name, ht_size(channel->members),
									 channel->topic));
				}
		}
	else
			...
			for(size_t i = 0; i < Vector_size(lines); i++)
				{
					List_push_back(
					  usr->msg_queue,
					  Server_create_message(serv, "705 %s %s :%s", usr->nick,
	...
	if(peer->registered)
		{
			List_push_back(
			  peer->msg_queue,
			  Server_create_message(serv, "462 %s :You may not reregister",
									peer->name));
			return;
		}
