		    ...
		    User * target_usr)
{
	List_push_back(usr->msg_queue,
		       Server_create_message(serv, RPL_WHOREPLY_MSG, usr->nick,
					     ...
					     target_usr->username,
					     target_usr->hostname,
					     serv->hostname, target_usr->nick,
					     "H", 0, target_usr->realname));
}
...
void send_motd_reply(Server * serv, User * usr)
{
	...
	if (motd) {
		List_push_back(usr->msg_queue,
			       Server_create_message(serv, RPL_MOTD_MSG,
						     usr->nick, motd));
	} else {
		List_push_back(usr->msg_queue,
			       Server_create_message(serv, ERR_NOMOTD_MSG,
						     usr->nick));
	}
...
{
	List_push_back(usr->msg_queue,
		       Server_create_message(serv, RPL_WELCOME_MSG, usr->nick,
					     usr->nick));
	List_push_back(usr->msg_queue,
...
void send_topic_reply(Server * serv, User * usr, Channel * channel)
{
	if (channel->topic) {
		List_push_back(usr->msg_queue,
			       Server_create_message(serv, RPL_TOPIC_MSG,
...
bool check_user_registration(Server * serv, User * usr)
{
	if (!usr->registered && usr->nick_changed && usr->username
		...
		// <realname>
		char *message =
		    Server_create_message(serv, "%s 1 %s %s 1 + :%s", usr->nick,
					  usr->username,
					  usr->hostname, usr->realname);
...
{
	char *subject =
	    make_string(":%s " RPL_NAMREPLY_MSG, serv->name, usr->nick,
			"=", channel->name);
	...
	while (ht_iter_next(&itr, NULL, (void **)&member)) {
		...
		size_t len = strlen(username) + 1;	// Length for name
		// and space
	...
	List_push_back(usr->msg_queue,
		       Server_create_message(serv, RPL_ENDOFNAMES_MSG,
					     usr->nick, channel->name));
}
...
void Server_handle_PRIVMSG(Server * serv, User * usr, Message * msg)
{
	...
	if (msg->n_params == 0) {
		List_push_back(usr->msg_queue,
			       Server_create_message(serv, ERR_NORECIPIENT_MSG,
						     usr->nick));
		return;
	}
	...
	if (target[0] == '#') {
		Channel *channel =
		    ht_get(serv->name_to_channel_map, target + 1);
...
void Server_handle_WHO(Server * serv, User * usr, Message * msg)
{
	...
	if (msg->n_params == 0) {
		List_push_back(usr->msg_queue,
			       Server_create_message(serv, RPL_ENDOFWHO_MSG,
						     usr->nick, ""));
		return;
	}
	...
	if (mask[0] == '#') {
		if (ht_contains(serv->name_to_channel_map, mask + 1)) {
			...
			Channel *channel
			    = ht_get(serv->name_to_channel_map, mask + 1);
			HashtableIter itr;
			ht_iter_init(&itr, channel->members);
			User *member = NULL;
			while (ht_iter_next(&itr, NULL, (void **)&member)) {
		...
		if (other_user) {
			for (size_t i = 0;
			     i < Vector_size(other_user->channels); i++) {
				Channel *channel =
				    ht_get(serv->name_to_channel_map,
					   Vector_get_at(other_user->channels,
							 i));
				if (channel) {
					send_who_reply(serv, usr, channel,
						       other_user);
				}
			}
		}
...
void Server_handle_LIST(Server * serv, User * usr, Message * msg)
{
	...
	// Reply start
	List_push_back(usr->msg_queue,
		       Server_create_message(serv, RPL_LISTSTART_MSG,
					     usr->nick));
	...
	if (msg->n_params == 0) {
		...
		while (ht_iter_next(&itr, NULL, (void **)&channel)) {
			assert(channel);
			List_push_back(usr->msg_queue,
				       Server_create_message(serv, RPL_LIST_MSG,
							     usr->nick,
							     channel->name,
							     ht_size(channel->
								     members),
		...
		while (tok) {
			...
			if (ht_contains(serv->name_to_channel_map, target)) {
				Channel *channel
				    = ht_get(serv->name_to_channel_map, target);
				assert(channel);
				List_push_back(usr->msg_queue,
					       Server_create_message(serv,
								     RPL_LIST_MSG,
...
void Server_handle_TOPIC(Server * serv, User * usr, Message * msg)
{
	...
	if (msg->body) {
		...
		log_info("user %s set topic for channel %s", usr->nick,
			 channel->name);
	} else {
...
void Server_handle_PART(Server * serv, User * usr, Message * msg)
{
	...
	Server_message_channel(serv, serv->name, channel_name,
			       broadcast_message);
...
void Server_handle_HELP(Server * serv, User * usr, Message * msg)
{
	...
	if (help) {
		List_push_back(usr->msg_queue,
			       Server_create_message(serv, "704 %s %s :%s",
						     usr->nick, subject,
						     help->title));
		List_push_back(usr->msg_queue,
			       Server_create_message(serv, "705 %s %s :",
						     usr->nick, subject));
	...
	List_push_back(usr->msg_queue,
		       Server_create_message(serv,
					     "524 %s %s :No help available on this topic",
					     usr->nick, subject));
}
...
void Server_handle_INFO(Server * serv, User * usr, Message * msg)
{
	...
	List_push_back(usr->msg_queue,
		       Server_create_message(serv, "371 %s :%s", usr->nick,
					     serv->info));
	List_push_back(usr->msg_queue,
		       Server_create_message(serv, "374 %s :End of INFO list",
					     usr->nick));
}
...
void check_peer_registration(Server * serv, Peer * peer)
{
	...
	if (peer->server_type == ACTIVE_SERVER) {
		char *other_passwd
		    = get_server_passwd(serv->config_file, peer->name);
	...
	while (ht_iter_next(&itr, NULL, (void **)&other_peer)) {
		if (other_peer->registered && !other_peer->quit) {
			List_push_back(peer->msg_queue,
				       Server_create_message(serv, "SERVER %s",
							     other_peer->name));
		}
	}
	...
	while (ht_iter_next(&itr, NULL, (void **)&other_channel)) {
		List_push_back(peer->msg_queue,
			       Server_create_message(serv, "MODE #%s",
						     other_channel->name));
	}
