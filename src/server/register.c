...
bool check_registration_complete(Server * serv, User * usr)
{
	...
	if (usr->nick_changed) {
		...
		User_add_msg(usr,
			     make_reply(":%s " RPL_WELCOME_MSG, serv->hostname,
					usr->nick, usr->nick));
		User_add_msg(usr,
			     make_reply(":%s " RPL_YOURHOST_MSG, serv->hostname,
					usr->nick, usr->hostname));
		User_add_msg(usr,
			     make_reply(":%s " RPL_CREATED_MSG, serv->hostname,
					usr->nick, serv->created_at));
		User_add_msg(usr,
			     make_reply(":%s " RPL_MYINFO_MSG, serv->hostname,
		...
		if (motd) {
			User_add_msg(usr,
				     make_reply(":%s " RPL_MOTD_MSG,
						serv->hostname, usr->nick,
						motd));
		} else {
			User_add_msg(usr,
				     make_reply(":%s " ERR_NOMOTD_MSG,
						serv->hostname, usr->nick));
		}
...
bool update_nick_map(Server * serv, User * usr)
{
	...
	if (!cc_hashtable_contains_key(serv->user_to_nicks_map, usr->username)) {
		...
		cc_hashtable_add(serv->user_to_nicks_map, strdup(usr->username),
				 arr);
		return true;
	}
	...
	if (cc_hashtable_get
	    (serv->user_to_nicks_map, usr->username,
	     (void **)&nicks) == CC_OK) {
		// Check if nick already exists in array
		for (size_t i = 0; i < cc_array_size(nicks); i++) {
			char *value = NULL;

			if (cc_array_get_at(nicks, i, (void **)&value) == CC_OK
			    && !strcmp(value, usr->nick)) {
				return true;
			}
		}
...
{
	if (!usr->username
	    || !cc_hashtable_contains_key(serv->user_to_nicks_map,
					  usr->username)) {
		return NULL;
	}
